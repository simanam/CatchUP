...
module.exports = {
   async createUser(req, res){
        ...
        return bcrypt.hash(value.password, 10, (err, hash) => {
            ...
            const body = {
                fname: Helpers.firstUpper(value.fname),
                lname: Helpers.firstUpper(value.lname),
    ...
    async loginUser(req, res){
        ...
        await User.findOne({username: Helpers.firstUpper(req.body.username)}).then(user => {
            ...
            return bcrypt.compare(req.body.password, user.password).then((result) => {
                if(!result){
                return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({message:'Incorrect Information'});
                }
                const token = jwt.sign({data: user}, dbConfig.secret, {
                    expiresIn: '1h'
                });
